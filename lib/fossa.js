...
exports.loadScripts = function loadScripts(app) {
    ...
    conf['services'].forEach(function (service) {
        ...
        function printTypeAndParams(type, params) {
            if (params) {
                console.log(('          GET: ').white + type.yellow + '  ' + JSON.stringify(params).cyan);
            } else {
                console.log(('          GET: ').white + type.yellow);
            }
        }
        ...
        if (get) {
            ...
            var type = get['type'];
            printTypeAndParams(type,params);
            var response = require('./scripts/' + type).get;
        ...
        if (post) {
            ...
            var type = post['type'];
            printTypeAndParams(type,params);
            var response = require('./scripts/' + type).post;
