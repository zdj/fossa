...
exports.put = function(req, res, cb, params) {
	...
	var amqpPublish = 'curl -i -u ' + params['user'] + ' -H "content-type:application/json" -XPUT -d \'{"configure":".*","write":".*","read":".*","configure":".*"}\' http://' + params['host'] + ':' + params['port'] + '/api/permissions/' + params['vhost'] + '/' + params['username'];
